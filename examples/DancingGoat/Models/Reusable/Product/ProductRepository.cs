...
namespace DancingGoat.Models
{
    ...
    public class ProductRepository : ContentRepositoryBase
    {
        ...
        public async Task<IEnumerable<IProductFields>> GetProducts(string languageName, IDictionary<string, TaxonomyViewModel> filter, bool includeSecuredItems = true, CancellationToken cancellationToken = default)
        {
            ...
            return await GetCachedQueryResult<IProductFields>(queryBuilder, options, cacheSettings, GetDependencyCacheKeys, cancellationToken);
        }
        ...
        private async Task<ISet<string>> GetDependencyCacheKeys(IEnumerable<IProductFields> products, CancellationToken cancellationToken)
        {
            ...
            await AddProductPageDependencies(products, dependencyCacheKeys, cancellationToken);

            return dependencyCacheKeys;
        }


        private async Task AddProductPageDependencies(IEnumerable<IProductFields> products, HashSet<string> dependencyCacheKeys, CancellationToken cancellationToken)
        {
            ...
            var productIds = products.Select(product => ((IContentItemFieldsSource)product).SystemFields.ContentItemID);
            var productCacheKeys = await linkedItemsDependencyRetriever.Get(productIds, 1, cancellationToken);

            foreach (var productId in productIds)
            {
                dependencyCacheKeys.Add(CacheHelper.GetCacheItemName(null, "contentitem", "byid", productId));
            }

            foreach (var key in productCacheKeys)
            {
                dependencyCacheKeys.Add(key);
            }
